...
- Optional DB storage for sent notifications via `cds.requires.notifications.storeNotifications: true`. When enabled, each notification is stored to the database after successful delivery to ANS, including properties and navigation target parameters.
